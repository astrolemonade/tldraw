...
describe('custom shape bounds snapping - translate', () => {
	...
	class TestShapeUtil extends BaseBoxShapeUtil<TestShape> {
		...
		override getSnapInfo(shape: TestShape) {
			return {
				boundsSnapPoints: shape.props.boundsSnapPoints ?? undefined,
			}
		}
	}
...
describe('custom handle snapping', () => {
	...
	beforeEach(() => {
		...
		ids = editor.createShapesFromJsx([
			<TL.line
				...
				y={0}
				handles={{
					start: { id: 'start', type: 'vertex', canSnap: true, index: 'a0', x: 0, y: 0 },
					end: { id: 'end', type: 'vertex', canSnap: true, index: 'a1', x: 100, y: 100 },
				}}
			/>,
			<TL.test ref="test" x={200} y={200} w={100} h={100} boundsSnapPoints={null} />,
		])
	...
	function startDraggingHandle() {
		...
		editor.pointerDown(100, 100, { target: 'handle', shape, handle: shape.props.handles.end })
	}

	function handlePosition() {
		const shape = editor.select(ids.line).getOnlySelectedShape()! as TLLineShape
		return { x: shape.props.handles.end.x, y: shape.props.handles.end.y }
	}
